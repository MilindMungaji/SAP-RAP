...
****************************************************************************
** DAY 18 - Implement EML Statement
****************************************************************************

    DATA : lt_create TYPE TABLE FOR CREATE zi_iws_student.  "Entity

*    lt_create = VALUE #( ( Rollno = 12 Dept = 'IT' Firstname = 'Anup' Lastname = 'Chaurasia' Birthdate = '20000101' age = 20
*                           %control = VALUE #( Rollno    = if_abap_behv=>mk-on
*                                               Dept      = if_abap_behv=>mk-on
*                                               Firstname = if_abap_behv=>mk-on
*                                               Lastname  = if_abap_behv=>mk-on
*                                               Birthdate = if_abap_behv=>mk-on
*                                               Age       = if_abap_behv=>mk-on ) ) ).
*
*    "EML Statement to create Student Entity
*
*    MODIFY ENTITIES OF zi_iws_student
*    "IN LOCAL MODE               "To bypass AUthorizations/ Checks
*    ENTITY Student
...
*      COMMIT ENTITIES.
*      out->write( 'Entity Create is successful..' ).
*    ENDIF.

*    DATA : lt_update TYPE TABLE FOR UPDATE zi_iws_student.  "Entity
*
*    MODIFY ENTITIES OF zi_iws_student
*    "IN LOCAL MODE               "To bypass AUthorizations/ Checks
*    ENTITY Student
*    update FROM VALUE #( ( Rollno = 12 Dept = 'CS' Firstname = 'Anup1' Lastname = 'Chaurasia1' Birthdate = '20000101' age = 20
*                           %control = VALUE #( Rollno    = if_abap_behv=>mk-on
*                                               Dept      = if_abap_behv=>mk-on
*                                               Firstname = if_abap_behv=>mk-on
...
*    IF lt_failed IS INITIAL.
*      COMMIT ENTITIES.
*      out->write( 'Entity update is successful..' ).
*    ENDIF.

**-- EML Statement to Delete Entity
*    MODIFY ENTITIES OF zi_iws_student
*    ENTITY Student
*    DELETE FROM VALUE #( ( rollno = 12  ) )
*    REPORTED DATA(lt_reported_create)
*    MAPPED DATA(lt_mapped_create)
*    FAILED DATA(lt_failed_create).
*
*    IF lt_failed_create IS INITIAL.
*      COMMIT ENTITIES.
*      out->write( 'Entity Deleted Successfully' ).
*    ENDIF.

**-- EML Statement to Read Entity
*    READ ENTITIES OF zi_iws_student
*    "IN LOCAL MODE
*    ENTITY Student
*    ALL FIELDS WITH VALUE #( ( Rollno = '7' ) )
